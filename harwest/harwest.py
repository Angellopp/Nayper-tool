...
def init():
  ...
  print("[1] Necesitamos crear un directorio para almacenar todos sus archivos\n",
        "   El directorio se creará como", os.getcwd() + os.path.sep + "<tu-input>")
  directory = input("> Ahora, ¿cual es el nombre de tu directorio? ")
  path = os.path.join(os.getcwd(), directory)
  print("\U0001F44D", "Tu directorio se creará en", path)
...
def main():
  ...
  if args.init or config_map is None:
    if config_map is None:
      print("Hola gris!", "\U0001F44B",
            "Parece que quieres tener tus problemas resueltos en tu repositorio GitHub"
            " Empecemos", "\N{rocket}", "\n")
    init()
  if 'func' in args:
    args.func(args)
